...
readingsRouter.post("/upload", upload.single("image"), async (req, res) => {
    const meterId = typeof req.body.meterId === "string" ? req.body.meterId : undefined;
    if (!meterId)
        throw new ValidationError("meterId is required");
    if (!req.file?.buffer)
    ...
    const anomalyReason = consumption === undefined ? undefined :
...
readingsRouter.post("/", validate(commitSchema), async (req, res) => {
    ...
    const anomalyReason = consumption === undefined ? undefined :
        consumption < 0 ? "Reading is lower than previous reading (possible meter reset)" :
            consumption > 450 ? `Unusually high consumption (${consumption} kWh)` :
                undefined;
    const status = req.body.source !== "MANUAL" && (req.body.confidenceScore ?? 1) < threshold
        ? "FLAGGED"
        : "ACCEPTED";
...
readingsRouter.get("/:id", async (req, res) => {
    ...
    if (req.user.role !== "ADMIN" && reading.userId !== req.user.id)
        throw new ForbiddenError();
