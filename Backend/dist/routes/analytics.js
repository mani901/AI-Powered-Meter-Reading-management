...
analyticsRouter.get("/admin/dashboard", requireRole("ADMIN"), async (_req, res) => {
    ...
    const [totalUsers, activeUsers, totalMeters, activeMeters, totalReadings, readingsThisMonth, flaggedReadings, pendingReviews, avgConfidence, revenueThisMonth,] = await Promise.all([
        prisma.user.count(),
    ...
    const monthlyReadingTrend = await Promise.all(Array.from({ length: 6 }).map(async (_v, idx) => {
    ...
    res.json({
        ...
        readingsBySource: readingsBySource.reduce((acc, r) => {
...
analyticsRouter.get("/admin/reports/user-growth", requireRole("ADMIN"), async (_req, res) => {
    const data = await Promise.all(Array.from({ length: 6 }).map(async (_v, idx) => {
        const d = addMonths(new Date(), -(5 - idx));
        const end = new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth() + 1, 1));
